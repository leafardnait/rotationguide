 ...
 * RspConfigCC class
 */
class RspConfigCC extends \Magento\Payment\Gateway\Config\Config
{
    ...
    public function sendApiPaymentTransaction($bodyRequest)
    {
        ...
        if (!$this->isActive()) {
            $this->_rspLogger->customLog('Rsp CC Payment Disable');
            return false;
        }
        try {
    ...
    public function getConfigValue($key)
    {
        ...
        return $this->_scopeConfig->getValue('payment/magekc_rsppayment_cc/'.$key, $storeScope);
    }
    ...
    {
        if (!empty($log)) {
            $this->_rspLogger->customLog(print_r($log, true));
        }
    }
}